...
# Reading data
data = pd.read_csv(r"D:/eng mariam/4th/ML/Tasks/Task2/multivariateData.dat", sep=',',header=None)
print(data)
...
# train_features1 = train_data[:,0]
# train_features2 = train_data[:,1]
# test_features1 = test_data[:,0]
# test_features2 = test_data[:,1]
X_0_train = np.ones((len(train_data), 1))
X_1_train = train_data.reshape(len(train_data), 2)
# X_2_train = train_features2.reshape(len(train_data), 1)
X_0_test = np.ones((len(test_data), 1))
X_1_test = test_data.reshape(len(test_data), 2)
# X_2_test = test_features2.reshape(len(test_data), 1)
